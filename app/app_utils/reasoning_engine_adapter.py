...
def attach_reasoning_engine_routes(app: FastAPI) -> None:
    ...
    def get_runtime() -> AdkApp:
        ...
        if runtime is None:
            ...
            # Monkeypatch google.auth.default to fallback to dummy credentials if it fails
                ...
                try:
                    ...
                    class DummyCredentials(Credentials):
                        ...
                        def refresh(self, request):
                            self.token = "dummy-token"
                    return DummyCredentials(), "dummy-project-id"
            ...
            if not project and not os.environ.get("GOOGLE_CLOUD_PROJECT"):
                try:
                    vertexai.init(project="dummy-project-id", location="us-east1")
                except Exception:
                    pass
