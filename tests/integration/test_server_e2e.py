...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    data = {
        ...
        "new_message": {"role": "user", "parts": [{"text": "Design a meal plan for breakfast using Keto Avocado and Egg Salad."}]},
        "streaming": True,
    }
...
def test_a2a_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    message = Message(
        ...
        role=Role.user,
        parts=[Part(root=TextPart(text="Design a meal plan for breakfast using Keto Avocado and Egg Salad."))],
    )
...
@pytest.mark.skipif(not has_gcp_credentials(), reason="GCP credentials not found")
def test_reasoning_engine_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    response = requests.post(
        ...
        json={
            ...
            "input": {"user_id": f"u-{uuid.uuid4()}", "message": "Design a meal plan for breakfast using Keto Avocado and Egg Salad."},
        },
        stream=True,
        timeout=60,
    )
